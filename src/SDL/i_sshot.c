...
static int write_png_palette(png_struct *png_ptr, png_info *info_ptr,
                             SDL_Surface *scr) {
  int i, result = -1;
  png_color *palette;
  ...
  if (palette) {
    ...
    png_set_PLTE(png_ptr, info_ptr, palette, scr->format->palette->ncolors);
...
static void fill_buffer_hicolor(SDL_Surface *scr, void *buffer) {
  ...
  for (y = SCREENWIDTH * SCREENHEIGHT; y > 0;
...
enum { SCREENSHOT_SDL_INDEXED, SCREENSHOT_SDL_HICOLOR, SCREENSHOT_SDL_MODES };
...
static const struct screenshot_sdl_func {
  ...
  void (*fill_buffer)(SDL_Surface *scr, void *buffer);
    ...
    {sizeof(char), fill_buffer_indexed},
    {sizeof(png_color), fill_buffer_hicolor},
};

// Write the contents of the surface to the PNG file
// Called from I_ScreenShot when the PNG file is ready to write to

static int screenshot_sdl(png_struct *png_ptr, png_info *info_ptr,
                          SDL_Surface *scr,
                          const struct screenshot_sdl_func *mode) {
  int y, result = -1;
  byte *pixel_data;
    ...
    if (png_ptr) {
      ...
      if (info_ptr) {
        ...
        png_set_IHDR(png_ptr, info_ptr, SCREENWIDTH, SCREENHEIGHT, 8,
                     (V_GetMode() == VID_MODE8) ? PNG_COLOR_TYPE_PALETTE
                                                : PNG_COLOR_TYPE_RGB,
        ...
        case VID_MODE8:
          if (write_png_palette(png_ptr, info_ptr, scr) >= 0)
            result =
                screenshot_sdl(png_ptr, info_ptr, scr,
