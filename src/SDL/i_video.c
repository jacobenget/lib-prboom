...
int use_doublebuffer =
    1; // Included not to break m_misc, but not relevant to SDL
#endif
int use_fullscreen;
...
void I_StartTic(void) {
  SDL_Event Event;
  {
    boolean should_be_grabbed =
        mouse_enabled && !(paused || (gamestate != GS_LEVEL) || demoplayback);
...
static void I_InitInputs(void) {
  ...
  if (!nomouse_parm)
    SDL_WarpMouse((unsigned short)(SCREENWIDTH / 2),
                  (unsigned short)(SCREENHEIGHT / 2));
...
static void I_UploadNewPalette(int pal) {
  ...
  SDL_SetPalette(SDL_GetVideoSurface(), SDL_LOGPAL | SDL_PHYSPAL,
...
void I_UpdateVideoMode(void) {
  ...
  if (V_GetMode() == VID_MODEGL) {
    ...
    screen = SDL_SetVideoMode(SCREENWIDTH, SCREENHEIGHT, gl_colorbuffer_bits,
                              init_flags);
  } else {
    screen = SDL_SetVideoMode(SCREENWIDTH, SCREENHEIGHT, V_GetNumPixelBits(),
                              init_flags);
  }
