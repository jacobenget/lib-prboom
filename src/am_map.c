...
static void AM_rotate(fixed_t *x, fixed_t *y, angle_t a, fixed_t xorig,
                      fixed_t yorig) {
  fixed_t tmpx;
...
static void AM_drawPlayers(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    if (playeringame[i]) {
      fixed_t x = p->mo->x >> FRACTOMAPBITS,
              y = p->mo->y >> FRACTOMAPBITS; // e6y
      if (automapmode & am_rotate)
  ...
  for (i = 0; i < numsectors; i++) {
    ...
    while (t) // for all things in that sector
    {
      ...
      if (mapcolor_rkey || mapcolor_ykey || mapcolor_bkey) {
        ...
        case 13: // jff  red key
          AM_drawLineCharacter(
              cross_mark, NUMCROSSMARKLINES,
              16 << MAPBITS, // e6y
              t->angle, mapcolor_rkey != -1 ? mapcolor_rkey : mapcolor_sprt, x,
              y);
          t = t->snext;
          continue;
        case 39:
        case 6: // jff yellow key
          AM_drawLineCharacter(
              cross_mark, NUMCROSSMARKLINES,
              16 << MAPBITS, // e6y
              t->angle, mapcolor_ykey != -1 ? mapcolor_ykey : mapcolor_sprt, x,
              y);
          t = t->snext;
          continue;
        case 40:
        case 5: // jff blue key
          AM_drawLineCharacter(
              cross_mark, NUMCROSSMARKLINES,
              16 << MAPBITS, // e6y
              t->angle, mapcolor_bkey != -1 ? mapcolor_bkey : mapcolor_sprt, x,
              y);
          t = t->snext;
          continue;
        default:
