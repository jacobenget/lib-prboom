  ...
  *holdstring = '\0'; // empty string to start with
  ...
  while (!dehfeof(fpin) && *inbuffer) /* && (*inbuffer != ' ') */
  {
    ...
    while (strlen(holdstring) + strlen(inbuffer) >
           (size_t)maxstrlen) // Ty03/29/98 - fix stupid error
    {
...
static boolean StrToInt(char *s, long *l) {
  return ((sscanf(s, " 0x%lx", l) == 1) || (sscanf(s, " 0X%lx", l) == 1) ||
          (sscanf(s, " 0%lo", l) == 1) || (sscanf(s, " %ld", l) == 1));
}
...
boolean deh_GetData(char *s, char *k, uint_64_t *l, char **strval,
                    FILE *fpout) {
  char *t;                    // current char
