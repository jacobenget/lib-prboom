...
static const int nstandard_iwads =
    sizeof standard_iwads / sizeof *standard_iwads;
  ...
  if (gamestate != GS_LEVEL) { // Not a level
    ...
    if (oldgamestate != GS_LEVEL) {
      ...
      borderwillneedredraw =
          menuactive && isborder && viewactive && (viewwidth != SCREENWIDTH);
    }
...
static void D_SetPageName(const char *name) { pagename = name; }
...
static void CheckIWAD(const char *iwadname, GameMode_t *gmode,
                      boolean *hassec) {
...
static void D_DoomMainSetup(void) {
  ...
  if (!M_CheckParm("-noload")) {
    ...
    for (i = 0; i < MAXLOADFILES * 2; i++) {
      const char *fname =
          (i < MAXLOADFILES) ? wad_files[i] : deh_files[i - MAXLOADFILES];
      char *fpath;
  ...
  if (!M_CheckParm("-nodeh"))
    if ((p = W_CheckNumForName("DEHACKED")) !=
        -1) // cph - add dehacked-in-a-wad support
