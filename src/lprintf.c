...
static void I_PrintStr(int xp, const char *cp, int count, BOOL scroll) {
  ...
  if (scroll) {
    ...
    ScrollWindowEx(con_hWnd, 0, -OemHeight, NULL, &rect, NULL, NULL,
...
int Init_ConsoleWin(void) {
  ...
  con_hWnd = CreateWindow(szConName, titlebuffer, WS_CAPTION | WS_POPUP, 0, 0,
  ...
  MoveWindow(con_hWnd, (scr_width - width) / 2, (scr_height - height) / 2,
             width, height, TRUE);
  GetClientRect(con_hWnd, &cRect);
