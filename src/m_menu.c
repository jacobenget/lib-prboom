...
enum { mouse_horiz, mouse_empty1, mouse_vert, mouse_empty2, mouse_end } mouse_e;

// The definitions of the Mouse Sensitivity menu

menuitem_t MouseMenu[] = {{2, "M_HORSEN", M_MouseHoriz, 'h'},
                          {-1, "", 0},
                          {2, "M_VERSEN", M_MouseVert, 'v'},
                          {-1, "", 0}};

menu_t MouseDef = {mouse_end, &OptionsDef, MouseMenu, M_DrawMouse, 60, 64, 0};
...
menuitem_t SetupMenu[] = {
    {1, "M_COMPAT", M_Compat, 'p'}, {1, "M_KEYBND", M_KeyBindings, 'k'},
    {1, "M_WEAP", M_Weapons, 'w'},  {1, "M_STAT", M_StatusBar, 's'},
    {1, "M_AUTO", M_Automap, 'a'},  {1, "M_ENEM", M_Enemy, 'e'},
    {1, "M_MESS", M_Messages, 'm'}, {1, "M_CHAT", M_ChatStrings, 'c'},
};
...
static void M_DrawSetting(const setup_menu_t *s) {
  ...
  if (flags & S_KEY) { // Key Binding
    ...
    if (key) {
      ...
      } else if (key == &key_up || key == &key_speed || key == &key_fire ||
                 key == &key_strafe) {
        if (s->m_mouse)
  ...
  if (flags & S_COLOR) // Automap paint chip
  {
    ...
    // uses a patch draw the paint chip
    V_FillRect(0, x * SCREENWIDTH / 320, (y - 1) * SCREENHEIGHT / 200,
               8 * SCREENWIDTH / 320, 8 * SCREENHEIGHT / 200, PAL_BLACK);
    V_FillRect(0, (x + 1) * SCREENWIDTH / 320, y * SCREENHEIGHT / 200,
               6 * SCREENWIDTH / 320, 6 * SCREENHEIGHT / 200, (byte)ch);
  ...
  if (flags & S_STRING) {
    ...
    if (setup_select && (s->m_flags & (S_HILITE | S_SELECT))) {
      ...
      *c = text[chat_index]; // hold temporarily
      ...
      V_FillRect(0, ((x + cursor_start - 1) * SCREENWIDTH) / 320,
                 (y * SCREENHEIGHT) / 200, (char_width * SCREENWIDTH) / 320,
                 9 * SCREENHEIGHT / 200, PAL_WHITE);
    }
...
setup_menu_t *stat_settings[] = {stat_settings1, NULL};
...
setup_menu_t *auto_settings[] = {auto_settings1, auto_settings2, NULL};
...
enum { general_mouse, general_joy, general_leds };

enum { general_wad1, general_wad2, general_deh1, general_deh2 };
...
setup_menu_t comp_settings3[] = // Compatibility Settings screen #2
    {{"All boss types can trigger tag 666 at ExM8",
      S_YESNO,
      m_null,
      C_X,
