...
typedef struct default_s {
   ...
   *  But, we may need to free any such strings stored on our heap, and in that
  ...
  struct {
    ...
    // should be free'd if *ppsz is ever changed to point to a different string)
    char *owned_value;
  } location;
  struct {
    int i;
...
// Document this
void M_AssignStringValue(default_t *defaul, const char *string);
// Document this
void M_AssignStringValueAndTakeOwnership(default_t *defaul, char *string);
