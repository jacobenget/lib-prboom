...
void P_UnArchiveWorld(void) {
  ...
  for (i = 0, sec = sectors; i < numsectors; i++, sec++) {
    ...
    get = (void *)((uintptr_t)get + sizeof sec->floorheight);
    memcpy(&sec->ceilingheight, get, sizeof sec->ceilingheight);
    get = (void *)((uintptr_t)get + sizeof sec->ceilingheight);
  ...
  for (i = 0, li = lines; i < numlines; i++, li++) {
    ...
    for (j = 0; j < 2; j++)
      if (li->sidenum[j] != NO_INDEX) {
        ...
        get = (void *)((uintptr_t)get + sizeof si->textureoffset);
        memcpy(&si->rowoffset, get, sizeof si->rowoffset);
        get = (void *)((uintptr_t)get + sizeof si->rowoffset);
...
static uintptr_t number_of_thinkers;
  ...
  for (th = thinkercap.next; th != &thinkercap; th = th->next)
    if (th->function == P_MobjThinker) {
      ...
      *save_p++ = tc_mobj;
      ...
      if (((mobj_t *)th)->lastenemy &&
          ((mobj_t *)th)->lastenemy->thinker.function == P_MobjThinker) {
        memcpy(save_p + sizeof(void *),
               &(((mobj_t *)th)->lastenemy->thinker.prev), sizeof(void *));
      }
...
void P_ArchiveMap(void) {
  ...
  CheckSaveGame(2 * sizeof zero + sizeof markpointnum +
                markpointnum * sizeof *markpoints + sizeof automapmode +
                sizeof one);
