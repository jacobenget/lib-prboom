...
static void P_LoadGLSegs(int lump) {
  ...
  for (i = 0; i < numsegs; i++) { // check for gl-vertices
    ...
    if (ml->linedef != (unsigned short)-1) // skip minisegs
    {
      ...
      segs[i].length = GetDistance(segs[i].v2->x - segs[i].v1->x,
                                   segs[i].v2->y - segs[i].v1->y);
  ...
  for (i = 0; i < numlines; i++) {
...

    {
      ...
      if ((ld->sidenum[1] == NO_INDEX) && (ld->flags & ML_TWOSIDED)) {
        ...
        // cph - print a warning about the bug
        lprintf(LO_WARN,
                "P_LoadLineDefs: linedef %d has two-sided flag set, but no "
                "second sidedef\n",
                numlines - i - 1);
      }
    }
...
static void P_CreateBlockMap(void) {
  ...
  blockmaplump =
      Z_Malloc(sizeof(*blockmaplump) * (4 + NBlocks + linetotal), PU_LEVEL, 0);
  // blockmap header
...
static void P_LoadBlockMap(int lump) {
  ...
  blocklinks =
      Z_Calloc(bmapwidth * bmapheight, sizeof(*blocklinks), PU_LEVEL, 0);
...
void P_SetupLevel(int episode, int map, int playermask, skill_t skill) {
  ...
  if (compatibility_level >= lxdoom_1_compatibility ||
      M_CheckParm("-force_remove_slime_trails") > 0)
