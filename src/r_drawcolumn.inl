...
#define GETCOL8_DEPTH(col)                                                     \
  (dither_colormaps[filter_getDitheredPixelLevel(x, y, fracz)]                 \
                   [GETCOL8_MAPPED(col)])
#else
...
#define GETCOL8(frac, nextfrac)                                                \
  GETCOL8_DEPTH(filter_getDitheredForColumn(x, y, frac, nextfrac))
#define GETCOL15(frac, nextfrac)                                               \
  filter_getFilteredForColumn15(GETCOL8_DEPTH, frac, nextfrac)
#define GETCOL16(frac, nextfrac)                                               \
  filter_getFilteredForColumn16(GETCOL8_DEPTH, frac, nextfrac)
#define GETCOL32(frac, nextfrac)                                               \
  filter_getFilteredForColumn32(GETCOL8_DEPTH, frac, nextfrac)
#elif (R_DRAWCOLUMN_PIPELINE & RDC_ROUNDED)
#define GETCOL8(frac, nextfrac)                                                \
  GETCOL8_DEPTH(filter_getRoundedForColumn(frac, nextfrac))
#define GETCOL15(frac, nextfrac)                                               \
  VID_PAL15(GETCOL8_DEPTH(filter_getRoundedForColumn(frac, nextfrac)),         \
            VID_COLORWEIGHTMASK)
#define GETCOL16(frac, nextfrac)                                               \
  VID_PAL16(GETCOL8_DEPTH(filter_getRoundedForColumn(frac, nextfrac)),         \
            VID_COLORWEIGHTMASK)
#define GETCOL32(frac, nextfrac)                                               \
  VID_PAL32(GETCOL8_DEPTH(filter_getRoundedForColumn(frac, nextfrac)),         \
            VID_COLORWEIGHTMASK)
#else
#define GETCOL8(frac, nextfrac) GETCOL8_DEPTH(source[(frac) >> FRACBITS])
#define GETCOL15(frac, nextfrac)                                               \
  VID_PAL15(GETCOL8_DEPTH(source[(frac) >> FRACBITS]), VID_COLORWEIGHTMASK)
#define GETCOL16(frac, nextfrac)                                               \
  VID_PAL16(GETCOL8_DEPTH(source[(frac) >> FRACBITS]), VID_COLORWEIGHTMASK)
#define GETCOL32(frac, nextfrac)                                               \
  VID_PAL32(GETCOL8_DEPTH(source[(frac) >> FRACBITS]), VID_COLORWEIGHTMASK)
#endif
...
static void R_DRAWCOLUMN_FUNCNAME(draw_column_vars_t *dcvars) {
  ...
  const fixed_t slope_texu =
      (dcvars->source == dcvars->nextsource) ? 0 : dcvars->texu & 0xffff;
#else
  ...
  if (dcvars->iscale > drawvars.mag_threshold) {
    R_GetDrawColumnFunc(R_DRAWCOLUMN_PIPELINE_TYPE, RDRAW_FILTER_POINT,
...
#ifdef RANGECHECK
  if (dcvars->x >= SCREENWIDTH || dcvars->yl < 0 || dcvars->yh >= SCREENHEIGHT)
...
#if (R_DRAWCOLUMN_PIPELINE & RDC_BILINEAR)
  frac =
      dcvars->texturemid - (FRACUNIT >> 1) + (dcvars->yl - centery) * fracstep;
#else
  frac = dcvars->texturemid + (dcvars->yl - centery) * fracstep;
#endif

  if (dcvars->drawingmasked &&
...
// do nothing else when drawin fuzz columns
#if (!(R_DRAWCOLUMN_PIPELINE & RDC_FUZZ))
  {
    ...
    const unsigned int filter_fracu =
        (dcvars->source == dcvars->nextsource) ? 0 : (dcvars->texu >> 8) & 0xff;
#else
...
#define FIXEDT_128MASK ((127 << FRACBITS) | 0xffff)
      while (count--) {
        *dest = GETDESTCOLOR(
            GETCOL(frac & FIXEDT_128MASK, (frac + FRACUNIT) & FIXEDT_128MASK));
        INCY(y);
        dest += 4;
        frac += fracstep;
      }
      ...
      if (!(dcvars->texheight & heightmask)) {     // power of 2 -- killough
        ...
        while ((count -= 2) >=
          ...
          *dest = GETDESTCOLOR(GETCOL(frac & fixedt_heightmask,
                                      (frac + FRACUNIT) & fixedt_heightmask));
          INCY(y);
          dest += 4;
          frac += fracstep;
          *dest = GETDESTCOLOR(GETCOL(frac & fixedt_heightmask,
