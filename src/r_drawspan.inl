...
#define GETDEPTHMAP(col)                                                       \
  dither_colormaps[filter_getDitheredPixelLevel(x1, y, fracz)][(col)]
#else
...
#define GETCOL_LINEAR(col)                                                     \
  filter_getFilteredForSpan15(GETDEPTHMAP, xfrac, yfrac)
#elif (R_DRAWSPAN_PIPELINE_BITS == 16)
#define GETCOL_POINT(col) VID_PAL16(GETDEPTHMAP(col), VID_COLORWEIGHTMASK)
#define GETCOL_LINEAR(col)                                                     \
  filter_getFilteredForSpan16(GETDEPTHMAP, xfrac, yfrac)
#elif (R_DRAWSPAN_PIPELINE_BITS == 32)
#define GETCOL_POINT(col) VID_PAL32(GETDEPTHMAP(col), VID_COLORWEIGHTMASK)
#define GETCOL_LINEAR(col)                                                     \
  filter_getFilteredForSpan32(GETDEPTHMAP, xfrac, yfrac)
#endif
...
static void R_DRAWSPAN_FUNCNAME(draw_span_vars_t *dsvars) {
...
#endif
  {
    ...
    SCREENTYPE *dest =
        drawvars.TOPLEFT + dsvars->y * drawvars.PITCH + dsvars->x1;
    ...
    while (count) {
      ...
      *dest++ = GETCOL(filter_getRoundedForSpan(xfrac, yfrac));
      ...
      const fixed_t xtemp =
          ((xfrac >> 16) +
           (filter_getDitheredPixelLevel(x1, y, ((xfrac >> 8) & 0xff)))) &
          63;
      const fixed_t ytemp =
          ((yfrac >> 10) +
           64 * (filter_getDitheredPixelLevel(x1, y, ((yfrac >> 8) & 0xff)))) &
          4032;
#else
