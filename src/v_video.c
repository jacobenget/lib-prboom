...
static void FUNC_V_CopyRect(int srcx, int srcy, int srcscrn, int width,
  ...
  src = screens[srcscrn].data + screens[srcscrn].byte_pitch * srcy +
        srcx * V_GetPixelDepth();
  dest = screens[destscrn].data + screens[destscrn].byte_pitch * desty +
         destx * V_GetPixelDepth();
  ...
  for (y = 0; y < SCREENHEIGHT; y += 64)
    for (x = y ? 0 : 64; x < SCREENWIDTH; x += 64)
      V_CopyRect(0, 0, scrn, ((SCREENWIDTH - x) < 64) ? (SCREENWIDTH - x) : 64,
                 ((SCREENHEIGHT - y) < 64) ? (SCREENHEIGHT - y) : 64, x, y,
                 scrn, VPT_NONE);
  ...
  if (V_GetMode() == VID_MODE8 && !(flags & VPT_STRETCH)) {
    ...
    for (dcvars.x = left; dcvars.x < right; dcvars.x++, col += DXI) {
      ...
      for (i = 0; i < column->numPosts; i++) {
        ...
        dcvars.yh =
            (((y + post->topdelta + post->length) * DY - (FRACUNIT >> 1)) >>
             FRACBITS);
...
static void WRAP_gld_CopyRect(int srcx, int srcy, int srcscrn, int width,
                              int height, int destx, int desty, int destscrn,
                              enum patch_translation_e flags) {}
...
static void NULL_FillRect(int scrn, int x, int y, int width, int height,
                          byte colour) {}
static void NULL_CopyRect(int srcx, int srcy, int srcscrn, int width,
                          int height, int destx, int desty, int destscrn,
                          enum patch_translation_e flags) {}
